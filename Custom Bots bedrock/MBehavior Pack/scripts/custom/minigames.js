...
// Maps are compact floating arenas. They are generated at a fresh coordinate
...
function buildMap(game) {
    ...
    // A thin air clearing keeps the floating arena playable without deleting
    // a large amount of terrain. The region is removed when the match ends.
    ...
    runMapCommand(game, `setblock ${centerX} ${baseY + 1} ${centerZ} minecraft:emerald_block`);
    runMapCommand(game, `setblock ${centerX + 2} ${baseY + 1} ${centerZ} minecraft:diamond_block`);
...
function buildGameTeams(game, combatants, mode) {
    ...
    for (let index = 0; index < combatants.length; index += 1) {
        ...
        const spawn = mode === "ffa"
            ? getRingSpawn(game.mapOrigin || game.origin, index, combatants.length, 7)
            : {
