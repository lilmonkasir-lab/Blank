...
function showBedWarsLaunchMenu(player, mapId) {
    const party = getPartyForPlayer(player);
    if (!party || party.hostId !== player.id) {
        safeMessage(player, "§cOnly the party host can start BedWars.");
        showMiniGamesMenu(player);
        return;
    }
    const map = getMapById(mapId);
    const form = new ActionFormData()
        .title("BedWars Ready")
        .body(`Winning map: ${map.name}\n\nPress Start BedWars to choose kits and bots, then launch the match.`)
        .button("Start BedWars")
        .button("Vote BedWars Map Again")
        .button("Back");
    form.show(player).then(response => {
        ...
        if (response.selection === 0) showGameOptions(player, "bedwars", map.id);
        else showMapVoteMenu(player);
    }).catch(() => {});
}
...
function showStartModeMenu(player) {
    ...
    form.show(player).then(response => {
        ...
        if (mode === "bedwars") {
            ...
            showBedWarsLaunchMenu(player, party.mapVoteSession.winnerId);
            return;
        }
...
export function showMiniGamesMenu(player) {
    ...
    const startBedWarsIndex = party.hostId === player.id && voteSession && voteSession.complete
        ? nextButtonIndex++
        : -1;
    if (startBedWarsIndex >= 0) {
        form.button(`Start BedWars: ${getMapById(voteSession.winnerId).name}`);
    }

    const voteButtonIndex = voteSession ? nextButtonIndex++ : -1;
    if (voteSession) form.button("Vote BedWars Map");
    const inviteButtonIndex = nextButtonIndex++;
    form.button("Invite Friend");
    const invitesButtonIndex = nextButtonIndex++;
    form.button(`Invitations (${invites.length})`);
    const leaveButtonIndex = nextButtonIndex;
    form.button("Leave Party");
    form.show(player).then(response => {
        if (response.canceled) return;
        if (response.selection === 0) showStartModeMenu(player);
        else if (response.selection === startBedWarsIndex) showBedWarsLaunchMenu(player, voteSession.winnerId);
        else if (response.selection === voteButtonIndex) showMapVoteMenu(player);
